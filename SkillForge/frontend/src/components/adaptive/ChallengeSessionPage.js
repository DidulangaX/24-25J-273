...
function ChallengeSessionPage() {
        ...
        if (currentQuestionIndex >= sessionData.questions.length) {
          ...
          if (submissionRes.ok) {
            ...
            if (
              ...
              (resultData.result && resultData.result.winner)
            ) {
              navigate(`/challenge-results/${sessionId}`);
            }
          }
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8051/api/adaptive/challenge/${sessionId}/submit`,
        {
          method: "POST",
          ...
          const checkInterval = setInterval(async () => {
            ...
            if (checkRes.ok) {
              ...
              if (
                ...
                (checkData.result && checkData.result.winner)
              ) {
  ...
  return (
    <div className="challenge-session-page container my-5">
      ...
      <div className="session-header mb-4">
        ...
        <p>
          ...
          <span className="badge bg-info">
            {sessionData.difficulty || "Standard"}
          </span>
        </p>
        <p>
          Time Remaining:{" "}
          <span className={getTimerClass()}>{formatTime(timer)}</span>
        </p>
        {questionsCount > 0 && (
      ...
      {currentQuestion ? (
        <div
          className={`question-card card mb-4 ${slideOut ? "slide-out" : ""}`}
        >
          ...
          <div className="card-body">
            <p>
              <strong>Prompt:</strong> {currentQuestion.prompt}
            </p>
            <textarea
        ...
        <div className="complete-message">
          ...
          <button
            ...
            onClick={() => navigate(`/challenge-results/${sessionId}`)}
          >
            View Results
          </button>
        </div>
