...
function ChallengeFriendsPage() {
  ...
  const fetchPendingChallenges = async () => {
    try {
      ...
      const res = await fetch(
        `${baseUrlAdaptive}/challenge/pending?user=${encodeURIComponent(
          currentUsername
        )}`,
        {
          headers: {
  ...
  const handleAcceptChallenge = async (sessionId) => {
    try {
      ...
      const res = await fetch(
        `${baseUrlAdaptive}/challenge/${sessionId}/accept`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (updatedSession.status === "active") {
        ...
        setStatusMsg(
          `Accepted. Waiting for others... (Session ${updatedSession._id})`
        );
      }
    ...
    <div className="challenge-friends-container">
      <div className="page-header">
        ...
        <p className="subtitle">
          Compete with other users in skill-based challenges
        </p>
      </div>
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((u) => (
                    <tr
                      ...
                      className={
                        selectedParticipants.includes(u.username)
                          ? "selected-row"
                          : ""
                      }
                    >
                      ...
                      <td>{u.email}</td>
                      <td>
                        {u.lastActive ? formatDate(u.lastActive) : "Unknown"}
                      </td>
                    </tr>
                  ))
                )}
                  ...
                  <div className="challenge-header">
                    <div className="creator-info">
                      <span className="creator-avatar">
                        {ch.creator.charAt(0).toUpperCase()}
                      </span>
